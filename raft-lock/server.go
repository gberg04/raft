...
func (ls *LockServer) applyToStateMachine(op *Op) {
	...
	fmt.Printf("%v operation recieved for %s \n", op.Type, op.Key)
	if ls.isLeader() {
		fmt.Printf("Event committed to channel \n")
		ls.events <- Event{
			Op:        op.Type,
	...
	originalLockOwner, _ := ls.store.getLockOwner(op.Key)

	fmt.Printf("Original lock owner: %d \n", originalLockOwner)

	if op.Type == UnlockOp {
		ls.store.unlock(op.Key, op.ClientId)
		fmt.Printf("%d removed from queue \n", op.ClientId)
	} else if op.Type == LockOp {
		ls.store.lock(op.Key, op.ClientId)
		fmt.Printf("%d added to queue \n", op.ClientId)
	}

	newLockOwner, _ := ls.store.getLockOwner(op.Key)
	fmt.Printf("New lock owner: %d \n", newLockOwner)

	if originalLockOwner != newLockOwner {
		ls.giveLockToOwner(op.Key, op.ClientId)
		fmt.Printf("Lock given to new owner \n")
	}
